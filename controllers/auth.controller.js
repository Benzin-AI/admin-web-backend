...
exports.signUp = async (req, res) => {
  ...
  try {
    console.log('Roles recibidos:', roles); // Verificar roles recibidos
    ...
    if (roles && roles.length > 0) {
      ...
      console.log('Roles encontrados:', foundRoles); // Verificar roles encontrados
  ...
  try {
    ...
    res.json({
      user: {
        ...
        name: userFound.name,
        role: userFound.roles.map(role => role.name).join(', ') // Asumiendo que `roles` tiene una propiedad `name`
      },
      token
    });
  } catch (err) {
